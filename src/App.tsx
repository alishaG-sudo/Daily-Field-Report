  ...
  return <main className="app-shell">
    ...
    {view === 'crew' ? <section className="crew-layout">
      {submitted ? <article className="success-card"><span className="success-mark">✓</span><p className="eyebrow">Report received</p><h1>Your report is sent.</h1><p>Sam Patel will review it in the morning. Your submission was recorded at {formatTime()}.</p><button className="primary-button" onClick={resetForm}>Submit another report</button><button className="text-button" onClick={() => setView('super')}>View project reports</button></article> : <>
        <header className="crew-heading"><div className="mobile-report-bar"><button type="button" onClick={() => setView('super')} aria-label="Open superintendent dashboard">‹</button><div><strong>Daily Report</strong><span>Lot 14 · {form.date}</span></div><span className="draft-pill">Draft</span></div><div className="mobile-progress"><span>Daily check-in</span><strong>{formValid ? 'Ready' : '2 of 4'}</strong><i><b style={{ width: formValid ? '100%' : '50%' }}></b></i></div><p className="eyebrow">Daily field report</p><h1>What did your crew accomplish?</h1><p>Complete this check-in before you leave site. It takes about 90 seconds.</p></header>
        <form className="crew-form" onSubmit={submitReport}>
          ...
          <div className="mobile-submit-bar"><span>{photos.length} photos attached</span><button className="primary-button" type="submit" disabled={!formValid}>Submit report <b>→</b></button></div>
        </form>
      </>}
